...
const IndexPage = () => {
  ...
  useEffect(() => {
    fetch("https://endorser.ch:3000/api/claim?claimContents=I%20am%20building%20a%20gifting%20society,%20in%20ways%20that%20fulfill%20me.")
      .then(r => r.json())
      .then(res => {
        const accepts = res.map(i=> (i.claimType=='AcceptAction' ? i.issuedAt : undefined)).filter(x=>x).sort()
        setPledgeData(accepts)
        return fetch("https://endorser.ch:3000/api/claim?claimContents=I%20am%20building%20a%20society%20based%20on%20giving,%20in%20ways%20that%20fulfill%20me.")
      })
      .then(r => r.json())
      .then(res => {
